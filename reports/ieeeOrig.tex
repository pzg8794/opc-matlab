...
%		should stand alone
%----------------------------------------------------------------------
\title{Implementation of Pong Game on Altera Board}

\author{Pauline Martial, pxm8395@rit.edu\member{RIT, Rochester (NY)}}
...
\begin{abstract} 
This paper presents the method used and the relevant technical details of the implementation of a Pong game on the FLEX 10K Altera FPGA on DE2 board. Besides the basic features of this game (two paddles moving vertically and trying not to let the ball get out of the screen), the implementation presented in this paper keeps track of the score of both players (human player using keyboard to control the right paddle VS computer). The game can be manually reset with or without a reset of the scores. Furthermore, every time the game is reset, the starting position of the ball is chosen randomly in order to make each game different. \\
...
\begin{keywords}
Pong game, FLEX 10K, Altera, DE2 board.
\end{keywords}
...
%----------------------------------------------------------------------
% SECTION III: TEST PLAN
%	strategy
%	exhaustively testing
%	what errors were caught
%	undetectable errors
%	timing/functionality tests
%	test correct behavior only
%	"testbench, waveforms, or hardware-only"
%----------------------------------------------------------------------
\section{Test Plan}
